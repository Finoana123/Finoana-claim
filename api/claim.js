// /api/claim.js - Claim via FlareSolverr (compatible avec proxy HTTP)
...
export default async function handler(req, res) {
  ...
  // Fonction robuste pour appeler FlareSolverr (gère le réveil Render)
  ...
  try {
    ...
    // 2. Charger faucet.php avec FlareSolverr
    ...
    // Ajouter le proxy UNIQUEMENT s'il est fourni et non vide
    if (proxy && proxy.trim() !== '') {
    ...
    // Fusionner les cookies (ceux de FlareSolverr + notre cookie de session)
    ...
    // 3. Pause de 20 secondes (simule un temps de lecture humain)
    ...
    // Token CSRF (d'abord dans les cookies, puis dans le HTML)
    ...
    // Si les données sont manquantes, on log le début du HTML pour debug
    if (!csrfToken || !hash || !captchaResponse) {
      console.error('HTML reçu (début) :', pageHtml.substring(0, 500));
    ...
    // 5. Construire le payload du claim
    ...
    console.log(`📬 Réponse claim: ${responseBody.substring(0, 200)}`);
